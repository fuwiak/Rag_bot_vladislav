...
**Миграции применяются автоматически!** При каждом деплое Backend и Telegram Bots сервисов:

1. **Сначала пытается Alembic:**
   - Проверьте логи при деплое: должно быть `Applying Alembic migrations...`
   - Если успешно: `Running upgrade ... -> 2025_12_24_bot_is_active`

2. **Если Alembic не может (например, проблема с ревизиями):**
   - Скрипт автоматически применяет миграцию вручную через SQL
   - Проверяет, существует ли колонка `bot_is_active` перед добавлением
   - Автоматически активирует существующие боты с токенами
   - В логах будет: `✅ Manual migration applied successfully!`

3. **Отдельный сервис для миграций НЕ нужен:**
   - Миграции применяются автоматически при старте каждого сервиса
   - Если миграция уже применена, скрипт просто пропустит её
   - Всё происходит автоматически, без ручного вмешательства

**Если миграция всё ещё не применяется:**
- Проверьте логи Backend и Telegram Bots сервисов при деплое
- Убедитесь, что сервисы имеют доступ к БД (проверьте `DATABASE_URL`)
