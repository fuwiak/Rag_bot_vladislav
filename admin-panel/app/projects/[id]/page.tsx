        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-lg p-6 max-w-md w-full">
            ...
            <form onSubmit={async (e) => {
              ...
              try {
                const backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:8000'
                const newUserProjectId = projectId
                ...
                if (response.ok) {
                  ...
                  const errorData = await response.json().catch(() => ({ detail: 'Ошибка добавления пользователя' }))
                  alert(errorData.detail || 'Ошибка добавления пользователя')
                }
              } catch (err) {
                alert('Ошибка подключения к серверу')
              } finally {
              ...
              <div className="mt-6 flex justify-end space-x-3">
                <button
                  ...
                  className="px-4 py-2 bg-fb-gray-dark hover:bg-gray-400 text-fb-text rounded-lg font-semibold transition-colors"
                >
                  Отмена
                </button>
                <button
